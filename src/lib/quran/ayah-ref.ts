...
/** عرض مختصر: سورة:آية + جزء/حزب/صفحة (لللوحات والجلسات). */
export function formatAyahPoint(globalIndex: number): string {
  ...
  return `س${surah}:آ${ayah} — ص${L.page} ج${L.juz} ح${L.hizb}`;
}

export function formatAyahRangeLine(startGlobal: number, endGlobal: number): string {
  const { surah: s1, ayah: a1 } = globalIndexToSurahAyah(Math.min(startGlobal, endGlobal));
  const { surah: s2, ayah: a2 } = globalIndexToSurahAyah(Math.max(startGlobal, endGlobal));
  const span = pageSpanForGlobalRange(startGlobal, endGlobal);
  return `من ${s1}:${a1} إلى ${s2}:${a2} (صفحات ${span.from}–${span.to})`;
}
