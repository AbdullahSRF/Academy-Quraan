...
### رسالة «لا يوجد اتصال بالإنترنت» على Vercel وهناك إنترنت

غالبًا كانت من **Service Worker + `navigateFallback`** إلى `offline.html`: أي فشل تنقل (مثل 500 أو RSC) يُعرض كصفحة «غير متصل». تم **إلغاء `navigateFallback`** في `next.config.ts`. بعد النشر: نفّذ **تحديثًا قويًا** أو امسح بيانات الموقع للنطاق لإزالة SW قديم.

- فحص خفيف: `GET /api/health` → `{ "ok": true }` (بدون DB).
- فحص قاعدة البيانات: `GET /api/health?db=1` أو `GET /api/health/database` → `database: "up"` أو `503` مع سبب واضح.
- إن ظهر **404** على `/api/health/database` فالنسخة على Vercel غالبًا **أقدم من الكود** (لم يُدفع آخر commit) — ادفع إلى Git وانتظر إعادة البناء؛ أو استخدم مؤقتًا **`/api/health?db=1`** بعد النشر.
